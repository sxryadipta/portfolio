...
const CategoryIcons = {
  'Web Development': (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-sec opacity-70">
...
const SkillsList = () => {
  ...
  const skills = {
    ...
    'Mobile Development': ['Mobile-friendly web apps', 'React Native mobile apps'],
    'UI/UX Design & Prototyping': [
  ...
  return (
    <div className="text-left pt-3 md:pt-9">
      <h3 className="text-white text-3xl md:text-4xl font-semibold md:mb-6">What I do?</h3>
      <ul className="space-y-4 mt-4 text-lg">
        {Object.entries(skills).map(([category, items]) => (
          <li key={category} className="w-full">
            <div
              onClick={() => toggleItem(category)}
              className="md:w-[400px] w-full bg-[#1414149c] rounded-2xl text-left hover:bg-opacity-80 transition-all border border-white-icon-tr cursor-pointer overflow-hidden"
            >
              <div className="flex items-center gap-3 p-4">
                {CategoryIcons[category]}
                <div className="flex items-center gap-2 flex-grow justify-between">
                  <div className="min-w-0 max-w-[200px] md:max-w-none overflow-hidden">
                    <span className="block truncate text-white text-lg">{category}</span>
                  </div>
                  <svg
              ...
              >
                <ul className="space-y-2 text-white-icon text-sm">
